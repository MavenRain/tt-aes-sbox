# SPDX-FileCopyrightText: 2026 Onyeka Obi
# SPDX-License-Identifier: Apache-2.0

"""
Exhaustive cocotb testbench for the AES S-Box.
Tests all 256 input values for both encrypt (SubBytes) and decrypt (InvSubBytes) modes.
Verifies round-trip: InvSubBytes(SubBytes(x)) == x for all x.
"""

import cocotb
from cocotb.clock import Clock
from cocotb.triggers import RisingEdge, ClockCycles

# AES Forward S-Box lookup table (FIPS 197, Figure 7)
SBOX_FWD = [
    ,
]

# AES Inverse S-Box lookup table (FIPS 197, Figure 14)
SBOX_INV = [
    ,
]


@cocotb.test()
async def test_forward_sbox_exhaustive(dut):
    """Test all 256 SubBytes values against FIPS 197 reference table."""
    cocotb.start_soon(Clock(dut.clk, 10, units="us").start())

    # Reset
    dut.rst_n.value = 0
    dut.ena.value = 1
    dut.ui_in.value = 0
    dut.uio_in.value = 0
    await ClockCycles(dut.clk, 10)
    dut.rst_n.value = 1
    await ClockCycles(dut.clk, 2)

    pass_count = 0
    fail_count = 0

    for i in range(256):
        dut.ui_in.value = i
        dut.uio_in.value = 0b01  # valid=1, mode=0 (encrypt)
        await ClockCycles(dut.clk, 1)  # wait for registered output

        result = int(dut.uo_out.value)
        expected = SBOX_FWD[i]
        valid_out = int(dut.uio_out.value) & 1

        if result != expected:
            dut._log.error(f"FWD MISMATCH: SubBytes(0x{i:02X}) = 0x{result:02X}, expected 0x{expected:02X}")
            fail_count += 1
        else:
            pass_count += 1

        assert valid_out == 1, f"Output valid should be 1, got {valid_out}"

    dut._log.info(f"Forward S-Box: PASS={pass_count} FAIL={fail_count}")
    assert fail_count == 0, f"{fail_count} forward S-Box mismatches"


@cocotb.test()
async def test_inverse_sbox_exhaustive(dut):
    """Test all 256 InvSubBytes values against FIPS 197 reference table."""
    cocotb.start_soon(Clock(dut.clk, 10, units="us").start())

    # Reset
    dut.rst_n.value = 0
    dut.ena.value = 1
    dut.ui_in.value = 0
    dut.uio_in.value = 0
    await ClockCycles(dut.clk, 10)
    dut.rst_n.value = 1
    await ClockCycles(dut.clk, 2)

    pass_count = 0
    fail_count = 0

    for i in range(256):
        dut.ui_in.value = i
        dut.uio_in.value = 0b11  # valid=1, mode=1 (decrypt)
        await ClockCycles(dut.clk, 1)  # wait for registered output

        result = int(dut.uo_out.value)
        expected = SBOX_INV[i]
        valid_out = int(dut.uio_out.value) & 1

        if result != expected:
            dut._log.error(f"INV MISMATCH: InvSubBytes(0x{i:02X}) = 0x{result:02X}, expected 0x{expected:02X}")
            fail_count += 1
        else:
            pass_count += 1

        assert valid_out == 1, f"Output valid should be 1, got {valid_out}"

    dut._log.info(f"Inverse S-Box: PASS={pass_count} FAIL={fail_count}")
    assert fail_count == 0, f"{fail_count} inverse S-Box mismatches"


@cocotb.test()
async def test_round_trip(dut):
    """Verify InvSubBytes(SubBytes(x)) == x for all x."""
    cocotb.start_soon(Clock(dut.clk, 10, units="us").start())

    # Reset
    dut.rst_n.value = 0
    dut.ena.value = 1
    dut.ui_in.value = 0
    dut.uio_in.value = 0
    await ClockCycles(dut.clk, 10)
    dut.rst_n.value = 1
    await ClockCycles(dut.clk, 2)

    fail_count = 0

    for i in range(256):
        # Forward: SubBytes(i)
        dut.ui_in.value = i
        dut.uio_in.value = 0b01  # valid=1, mode=0
        await ClockCycles(dut.clk, 1)

        fwd_result = int(dut.uo_out.value)

        # Inverse: InvSubBytes(SubBytes(i))
        dut.ui_in.value = fwd_result
        dut.uio_in.value = 0b11  # valid=1, mode=1
        await ClockCycles(dut.clk, 1)

        inv_result = int(dut.uo_out.value)

        if inv_result != i:
            dut._log.error(f"ROUND-TRIP FAIL: InvSBox(SBox(0x{i:02X})) = 0x{inv_result:02X}, expected 0x{i:02X}")
            fail_count += 1

    dut._log.info(f"Round-trip test: {256 - fail_count}/256 passed")
    assert fail_count == 0, f"{fail_count} round-trip failures"


@cocotb.test()
async def test_reset_clears_output(dut):
    """Verify reset clears output registers."""
    cocotb.start_soon(Clock(dut.clk, 10, units="us").start())

    dut.rst_n.value = 0
    dut.ena.value = 1
    dut.ui_in.value = 0xFF
    dut.uio_in.value = 0b01
    await ClockCycles(dut.clk, 5)

    assert int(dut.uo_out.value) == 0x00, "Output should be 0 during reset"
    assert (int(dut.uio_out.value) & 1) == 0, "Valid should be 0 during reset"

    dut.rst_n.value = 1
    await ClockCycles(dut.clk, 2)
